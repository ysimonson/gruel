...
# Generate the library crate listing for the /hacking/ page from the workspace
# so it can't drift from reality. Exclusions match the `crate-docs` Makefile
# target — the source of truth for which crates get rustdoc'd.
echo "Generating crate listing..."
mkdir -p website/data
{
    echo '# Auto-generated by website/build.sh from crates/*/Cargo.toml.'
    echo ''
    for cargo in crates/*/Cargo.toml; do
        crate_dir=$(dirname "$cargo")
        name=$(basename "$crate_dir")
        case "$name" in
            gruel|gruel-runtime|gruel-spec|gruel-test-runner) continue ;;
        esac
        [ -f "$crate_dir/src/lib.rs" ] || continue
        description=$(awk -F' = ' '/^description *=/ {sub(/^"/, "", $2); sub(/" *$/, "", $2); print $2; exit}' "$cargo")
        printf '[[crates]]\nname = "%s"\nrustdoc_path = "%s"\ndescription = "%s"\n\n' \
            "$name" "${name//-/_}" "$description"
    done
} > website/data/crates.toml
