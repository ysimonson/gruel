...
/// 3. Passing the return value to `__gruel_exit`
///
/// # ABI
///
/// ```text
...
///     call __gruel_exit   ; exit (never returns)
/// ```
...
crate::define_for_all_platforms! {
    ...
    ///
    /// This is the main entry point called by Gruel-generated code when `main()`
    /// returns. The return value of `main()` becomes the exit code.
    ///
    /// # ABI
    ///
    /// ```text
    /// extern "C" fn __gruel_exit(status: i32) -> !
    /// ```
    ///
