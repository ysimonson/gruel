...
impl<'a> Emitter<'a> {
    ...
    ///
    /// This sets up RBP-based stack frame, saves callee-saved registers BEFORE rbp
    /// (so they can be restored after pop rbp in the epilogue), allocates space for
    /// locals and parameters, and saves incoming parameters:
    /// ```asm
    /// push r12             ; save callee-saved registers BEFORE rbp
    /// push r13
    /// ...
    /// push rbp
    /// mov rbp, rsp
    ...
    /// pop rbp              ; restore rbp
    /// pop r13              ; restore callee-saved in reverse order (emitter adds these)
    /// pop r12
    /// ret
    /// ```
    fn emit_prologue(&mut self) {
        // Save callee-saved registers BEFORE rbp
        // This allows the epilogue to restore them AFTER popping rbp
        // We need to clone because we iterate and also store
        ...
        // - num_params slots for saved parameters
        // Each slot is 8 bytes, total aligned to 16
        // Note: callee-saved + rbp are pushed, need to account for alignment
        let total_slots = self.num_locals + self.num_params;
        // Pushes so far: callee_saved.len() + 1 (rbp)
        let pushes_so_far = callee_saved.len() as i32 + 1;
        let needed_bytes = total_slots as i32 * 8;
        // For 16-byte alignment: (pushes_so_far * 8 + needed_bytes + sub_amount) % 16 == 0
        // Since pushes_so_far * 8 might not be aligned, we adjust
        let stack_size = ((needed_bytes + 15) / 16) * 16;
        ...
        // Parameters are stored after locals: slot num_locals + i for param i.
    ...
    fn emit_inst(&mut self, inst: &X86Inst) {
        match inst {
            ...
            X86Inst::Ret => {
                // If we have callee-saved registers but no frame from lowerer,
                // we still need to emit the epilogue to restore them
                // The prologue pushed callee-saved THEN rbp, so epilogue pops rbp THEN callee-saved
                // But if the MIR has no epilogue (no mov rsp,rbp; pop rbp before ret),
                // we need to emit the callee-saved pops here
                // Note: If the MIR DID have epilogue, the callee-saved pops were already
                // added after the pop rbp instruction.
                ...
                if !self.callee_saved.is_empty() && self.num_locals == 0 && self.num_params == 0 {
                    // Emit minimal epilogue: pop rbp, then pop callee-saved
                    // (callee-saved were pushed first, then rbp, in prologue)
            ...
            X86Inst::Pop { dst } => {
                ...
                // After popping rbp, we need to pop the callee-saved registers
                // that were pushed BEFORE rbp in the prologue
                if dst.as_physical() == Reg::Rbp && !self.callee_saved.is_empty() {
