...
impl<'a> Sema<'a> {
    ...
    fn analyze_ptr_cast_intrinsic(
        ...
        let result_type = Self::get_resolved_type(ctx, inst_ref, span, "@ptr_cast intrinsic")?;
        if !matches!(
            result_type.kind(),
            TypeKind::PtrMut(_) | TypeKind::PtrConst(_)
        ) && !result_type.is_error()
        {
            return Err(CompileError::new(
                ErrorKind::IntrinsicTypeMismatch(Box::new(IntrinsicTypeMismatchError {
                    name: "ptr_cast".to_string(),
                    expected: "Ptr(T) or MutPtr(T) (inferred from context)".to_string(),
                    found: self.format_type_name(result_type),
                })),
                span,
            ));
        ...
        let args_start = air.add_extra(&[p.air_ref.as_u32()]);
        let name = self.interner.get_or_intern_static("ptr_cast");
        let air_ref = air.add_inst(AirInst {
            data: AirInstData::Intrinsic {
                name,
                args_start,
