...
mod tests {
    ...
    fn test_format_error_with_span() {
        ...
        assert!(output.contains("test.gruel"));
    }
    ...
    fn test_json_format_error() {
        ...
        assert_eq!(json_diag.spans[0].file, "test.gruel");
    ...
    fn test_multi_file_formatter_single_file() {
        let source = "fn main() -> i32 { 1 + true }";
        let sources = vec![(FileId::new(1), SourceInfo::new(source, "test.gruel"))];
        let formatter = MultiFileFormatter::new(sources);
    ...
    fn test_multi_file_formatter_multiple_files() {
        ...
        let sources = vec![
            (FileId::new(1), SourceInfo::new(source1, "main.gruel")),
            (FileId::new(2), SourceInfo::new(source2, "helper.gruel")),
        ];
        let formatter = MultiFileFormatter::new(sources);
        ...
        assert!(output.contains("main.gruel"));
        assert!(output.contains("helper.gruel"));
    ...
    fn test_multi_file_json_formatter_multiple_files() {
        ...
        // Primary span should be from main.gruel
        let primary = json_diag.spans.iter().find(|s| s.primary).unwrap();
        assert_eq!(primary.file, "main.gruel");

        // Secondary span should be from helper.gruel
        let secondary = json_diag.spans.iter().find(|s| !s.primary).unwrap();
        assert_eq!(secondary.file, "helper.gruel");
        assert_eq!(
            secondary.label,
