...
impl<'a> CfgLower<'a> {
        ...
        match &inst.data {
            ...
            CfgInstData::Eq(lhs, rhs) => {
                ...
                if self.ctx.is_builtin_string(lhs_ty) {
                    // String equality: call __gruel_str_eq(ptr1, len1, ptr2, len2)
            ...
            CfgInstData::Ne(lhs, rhs) => {
                ...
                if self.ctx.is_builtin_string(lhs_ty) {
                    // String inequality: call __gruel_str_eq and invert result
                    let vreg = self.emit_string_eq_call(*lhs, *rhs);
                    self.value_map.insert(value, vreg);
    ...
    fn emit_comparison(&mut self, value: CfgValue, lhs: CfgValue, rhs: CfgValue, cond: Cond) {
        ...
        if self.ctx.is_builtin_string(lhs_ty) {
            // String comparison requires calling __gruel_str_eq runtime function
            ...
            // Set up arguments for __gruel_str_eq(ptr1, len1, ptr2, len2)
            ...
            // Call __gruel_str_eq
            let symbol_id = self.intern_symbol("__gruel_str_eq");
            self.mir.push(Aarch64Inst::Bl { symbol_id });
    ...
    /// Emit a call to __gruel_str_eq for string comparison.
    ///
