...
impl<'ctx, 'a> FnCodegen<'ctx, 'a> {
                ...
                let call_args: Vec<inkwell::values::BasicMetadataValueEnum<'ctx>> = args
                    ...
                    .filter_map(|arg| {
                        if arg.is_mut_ref() || arg.is_ref() {
                            ...
                            let inst = self.cfg.get_inst(arg.value).clone();
                            return match inst.data {
                                ...
                                // ADR-0082: when the receiver of a method
                                // call is a field access (`self.bytes` /
                                // `w.inner`), the AIR is `PlaceRead {
                                // place: ... }`. Build the GEP chain into
                                // the parent's storage and pass that
                                // pointer through unchanged so the
                                // callee mutates the actual field, not a
                                // copy. Mirrors `vec_recv_ptr` in the
                                // legacy intrinsic path.
                                CfgInstData::PlaceRead { ref place } => {
                                    let arg_ty = self.cfg.get_inst(arg.value).ty;
                                    if let Some(ptr) = self.build_place_gep_chain(place, arg_ty) {
                                        Some(inkwell::values::BasicMetadataValueEnum::from(ptr))
                                    } else {
                                ...
                                // ADR-0082: `&x` / `&mut x` already
                                // produces a pointer to `x`'s storage.
                                // For a Ref/MutRef param, pass it
                                // through directly — wrapping in a
                                // temp alloca would make the callee
                                // see `**T`, reading the first 8
                                // bytes of `x` as the call pointer.
                                CfgInstData::MakeRef { .. } => {
                                    let val = self.get_value(arg.value);
                                    Some(inkwell::values::BasicMetadataValueEnum::from(val))
                                }
                                _ => {
    ...
    fn translate_intrinsic(
        ...
        match id {
            ...
            // ============================================================
            // ADR-0066 + ADR-0082: Vec(T) construction syntactic
            // intrinsics. Method-level intrinsics (`vec_push`, `vec_pop`,
            // …) retired with ADR-0082 — bodies live in
            // `prelude/vec.gruel`.
