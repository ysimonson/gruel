...
- [~] **Phase 4: AIR caching with comptime side-effects replay** — Infrastructure landed across four sub-commits: 4a/4b serde derives throughout `gruel-air` (Type, InternedType, AnalyzedFunction, AirInst/AirInstData, AirPlace, AirPattern, StructDef/EnumDef/InterfaceDef, etc.); 4c custom Serialize/Deserialize for `TypeInternPool` that snapshots `Vec<TypeData>` and reconstructs structural-dedup HashMaps on load (round-trip test verifies same `InternedType` values for identical type expressions); 4d `CachedAirOutput` envelope in `gruel-cache::wire_air` with `InternerSnapshot` + functions + type pool + strings/bytes + interface defs/vtables + `comptime_dbg_output`. **Still pending to make the cache actually intercept builds:** TypeId remap walker (visits every Type/InternedType/StructId/EnumId/InterfaceId in cached AIR), `DiagnosticWrapper<WarningKind>` serde for warning replay, and pipeline integration into `CompilationUnit::analyze`. Each is its own focused chunk.
