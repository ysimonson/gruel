...
- [ ] **Phase 3: Signature fingerprinting** — Compute `sig_fp(file)` after sema for each file. Stable canonical encoding of pub items with bodies stripped. Add a golden-test suite to lock the encoding (changing the encoding would invalidate the world). No caching yet — this phase just produces and verifies the hash.

- [ ] **Phase 4: AIR caching with comptime side-effects replay** — Cache per-file AIR (analyzed functions + type pool entries + comptime output buffer) keyed by `air_key`. Implement the comptime side-effects replay path so AIR cache hits emit the original `@dbg`/warning output to stderr. Requires solving `TypeId` remapping on load. This is the trickiest phase; budget for an incremental-AIR fuzz target that compares cached vs. uncached output (both AIR and stderr) for equivalence.

- [ ] **Phase 5: Per-function bitcode caching** — Cache pre-optimization LLVM bitcode per function, keyed by `bitcode_key`. On hit, parse the cached bitcode and `link_in_module` it into the build's whole-program module instead of translating from AIR. On miss, translate from AIR as today and write the resulting bitcode to the cache. The LLVM optimizer pipeline and object emission still run on the assembled module — not cached.

- [ ] **Phase 6: Observability and `gruel cache` subcommand** — `cache stats` (size, entry count, hit rate from last build), `cache clean` (wipe cache directory), structured tracing events from `gruel-cache`. No automatic GC — see "Cache lifecycle" in the Decision section.

- [ ] **Phase 7: Cold-vs-hot benchmarks on the perf dashboard** — Wire up the scenarios listed in "Benchmarking and validation" as a new benchmark family in the perf-dashboard infrastructure (ADR-0043). Each scenario reports both wall-clock time and per-stage cache hit rate. Land the benchmarks and let them run over multiple iterations to establish baselines, surface determinism issues, and confirm the cold-cache-overhead claims hold across changes to the compiler. The feature stays gated behind `--preview incremental_compilation` and **off by default** through this phase.
