...
- [ ] Lex/parse the thirteen type names (`c_schar`, `c_uchar`, `c_short`, `c_ushort`, `c_int`, `c_uint`, `c_long`, `c_ulong`, `c_longlong`, `c_ulonglong`, `c_float`, `c_double`, `c_void`) as primitive type names. Lives alongside `i8`–`i64`, not contextual keywords.
- [ ] RIR + AIR: model the twelve arithmetic types so each carries (signedness, target-resolved width, "is float?") on its primitive entry. Model `c_void` as a separate "incomplete primitive" variant — no width, no alignment, no values, only usable inside `Ptr(_)`/`MutPtr(_)`. Choose between a `Type::CNamed { kind: CNamedKind }` shape and per-type variants based on which composes cleanest with the existing `Type` representation; the variant-vs-tagged-int decision is a Phase-1 implementation choice, not a normative one.
- [ ] Type resolution for the twelve arithmetic types: each resolves per the table above. Resolution lives in `gruel-target` next to `Triple::host()` so a single per-target query returns `(width, alignment, signedness, is_float)`. Bake all twelve into one `c_primitive_widths(target)` function so future targets (LLP64 Windows etc.) only need one map updated.
- [ ] Sema: literals coerce to each arithmetic C named type in `let`-bound and return contexts (integer literals → C named integer types; float literals → `c_float`/`c_double`). `as` casts between any arithmetic C named type and any other arithmetic type permitted. Binary ops require matching types (no implicit `c_int <-> i32`, no `c_long <-> c_longlong` even when they happen to share width).
- [ ] Sema for `c_void`: reject `c_void` in any value-bearing position (let-bound, parameter, return, struct field by value, enum variant by value). Permit only inside `Ptr(_)`/`MutPtr(_)`. Reject `*p` deref on `Ptr(c_void)`/`MutPtr(c_void)`; reject pointer arithmetic.
- [ ] Sema for pointer casts: `MutPtr(T) as MutPtr(c_void)` / `MutPtr(c_void) as MutPtr(T)` and the const variants are legal for any `T`. Lowers to a no-op LLVM bitcast.
- [ ] Sema for fn-pointer cast: `@mark(c) fn` identifiers in expression position coerce to `MutPtr(c_void)` via `as`. The resulting value carries the fn's address with C calling convention. The reverse direction (`MutPtr(c_void) as <fn type>`) is *not* in this ADR — flagged in Future Work as part of the typed extern fn pointer story.
- [ ] Codegen: each arithmetic type lowers to the LLVM type from the target-resolved width row. `pointer_sized_int_type()` not involved; none of the C named arithmetic types are pointer-sized (`isize`/`usize` cover that). `Ptr(c_void)`/`MutPtr(c_void)` lower to opaque pointer types (LLVM 15+ `ptr`).
- [ ] FFI allowed-types table widens to include all twelve arithmetic types plus `Ptr(c_void)` / `MutPtr(c_void)`.
- [ ] Spec: extend `03-types/01-integer-types.md` and `03-types/11-floating-point-types.md` with the C named arithmetic table; add `10-c-ffi/03-c-named-types.md` summarising the FFI rules and `c_void`'s incomplete-type rules in one place.
- [ ] Spec tests under `cases/types/c-named.toml`: literal coercion, explicit cast, arithmetic, signed/unsigned-mismatch rejection, FFI roundtrip per type. Group by underlying width to keep the file manageable. Add `cases/types/c-void.toml`: pointer-only-use enforcement, deref rejection, cast roundtrip, `@mark(c) fn` to `MutPtr(c_void)` cast.
- [ ] Preview gate `c_ffi_extras` fires on any C named type token (including `c_void`) and on the `@mark(c) fn as MutPtr(c_void)` cast.
