...
- [x] **Phase 6: Differential fuzzer** — Add `ComptimeProgram` generator to `gruel-fuzz` (produces programs with `@dbg` calls, no I/O, no strings, deterministic), add `comptime_differential` fuzz target that compares comptime `@dbg` buffer output against runtime stdout, add CI integration.
